**License**

This repository contains both educational training material and
software utilities.

Different parts of the repository are licensed under different licenses.


**Documentation and Training Material**


All documentation, figures, and Jupyter notebooks are licensed under:

Creative Commons Attribution 4.0 International (CC BY 4.0)

This allows sharing and adaptation provided that proper attribution
is given to the original author.

::::{dropdown} License for Documentation

:::{include} LICENSE-DOCUMENTATION.md
::::


**Code**


Python utilities and scripts are licensed under the MIT License.


::::{dropdown} License for Code

:::{include} LICENSE-CODE.md
::::


**Attribution Requirement**


If you reuse material from this repository in documentation,
courses, or derivative works, please include an attribution such as:

    Based on training material developed by Silvia Mazzoni
    for DesignSafe-CI, Texas Advanced Computing Center,
    University of Texas at Austin
    Licensed under CC BY 4.0

Full license texts are described above.

::::{dropdown} Repository Attribution Notice

:::{include} NOTICE.md
::::


**Non-Endorsement**


Reuse of this material must not imply endorsement by DesignSafe-CI,
Texas Advanced Computing Center, the University of Texas at Austin,
or the author.